"""
專門針對 Set C 進行超參數調整（優化版 v2）

優化重點：
1. 綁定所有 LSTM 層使用相同的單元數
2. 提高 dropout 下限至 0.25，增強正規化
3. 加入更小的學習率 (0.00005, 0.00001)
4. 增加 Early Stopping patience 至 15
...
from src.tuning.hyperparameter_tuner import save_tuner_results, compare_with_baseline
...
def build_model_setC_v2(hp):
    """
    Set C 優化版模型建構函數

    主要改進：
    1. 綁定單元數：所有 LSTM 層使用相同的 units
    2. 提高 dropout 下限至 0.25
    3. 擴展學習率搜尋範圍
    """
    if keras is None:
    ...
    units_per_layer = [units] * num_layers  # 所有層使用相同值

    # 5. Dropout 比率（提高下限，增強正規化）
...
def main():
    ...
    # 設置 logger
    logger = setup_logger("tuning_setC_v2", log_dir="logs/tuning_logs")

    logger.info("=" * 80)
    logger.info("Set C 專用超參數調整（優化版 v2）")
    logger.info("=" * 80)
    ...
    logger.info("")
    logger.info("優化重點:")
    logger.info("  1. 綁定所有 LSTM 層使用相同單元數")
    logger.info("  2. Dropout 下限提高至 0.25")
    logger.info("  3. 加入更小學習率 (0.00005, 0.00001)")
    logger.info("  4. Early Stopping patience 提高至 15")
    logger.info("=" * 80)
    ...
    # 6. 建立自訂 Tuner
    logger.info("\n步驟 6/7: 建立 Keras Tuner（Set C v2）")
    ...
    tuner = kt.BayesianOptimization(
        hypermodel=build_model_setC_v2,
        objective=kt.Objective("val_loss", direction="min"),
        max_trials=MAX_TRIALS,
        directory="logs/tuning_logs",
        project_name=PROJECT_NAME,
        overwrite=False,  # 保留舊試驗並繼續
    )
    ...
    logger.info("  - 單元數（綁定）: 32, 64, 128, 256")
    logger.info("  - Dropout: 0.25, 0.3, 0.35, 0.4, 0.45, 0.5")
    logger.info("  - 學習率: 0.001, 0.0005, 0.0001, 0.00005, 0.00001")
    logger.info("")
    logger.info("理論組合數: 4 × 3 × 4 × 6 × 5 = 1,440 種")
    ...
    # 執行搜尋（patience 提高至 15）
    callbacks = [
        keras.callbacks.EarlyStopping(
            monitor="val_loss",
            patience=15,  # 從 10 提高至 15
    ...
    logger.info("\n開始超參數搜尋...")
    logger.info(f"Early Stopping patience: 15（提高以應對 val_loss 波動）")
    ...
    stats = {
        ...
        "best_hyperparameters": {
            ...
            "units": best_hps.get("units"),  # 綁定單元數
            "units_per_layer": [best_hps.get("units")] * best_hps.get("num_layers"),
    ...
    # 儲存調整結果
    results_file = f"logs/tuning_logs/tuning_results_setC_v2_{timestamp}.txt"
    ...
    if Path(BASELINE_MODEL).exists():
        ...
        try:
            ...
            # 儲存比較報告
            comparison_file = f"logs/tuning_logs/comparison_report_setC_v2_{timestamp}.txt"
            with open(comparison_file, "w", encoding="utf-8") as f:
                f.write("=" * 80 + "\n")
                f.write("Set C 優化模型比較報告（v2）\n")
                f.write("=" * 80 + "\n\n")
                f.write(f"調整時間: {timestamp}\n")
                f.write(f"參考模型: {BASELINE_MODEL}\n")
                f.write(f"優化模型: {model_file}\n\n")
                f.write("優化策略:\n")
                f.write("  1. 綁定所有 LSTM 層使用相同單元數\n")
                f.write("  2. Dropout 下限提高至 0.25\n")
                f.write("  3. 學習率範圍擴展至 0.00001\n")
                f.write("  4. Early Stopping patience 提高至 15\n\n")
                f.write("=" * 80 + "\n")
                f.write("效能比較（測試集）\n")
                f.write("=" * 80 + "\n\n")
                f.write(f"參考模型:\n")
    ...
    logger.info("\n" + "=" * 80)
    logger.info("✅ Set C 專用超參數調整（v2）完成！")
    logger.info("=" * 80)
