"""
從 Keras Tuner 中斷的調整中取得目前最佳模型（v2 版本）
...
def main():
    """主程式"""
    # 設定參數（與 tune_setC_v2.py 相同）
    PROJECT_NAME = "lstm_stock_tuning_setC_v2"
    DIRECTORY = "logs/tuning_logs"

    logger = setup_logger("get_best_v2", log_dir="logs")

    print("=" * 80)
    print("從 Keras Tuner 取得目前最佳模型（v2）")
    print("=" * 80)
    ...
    if not tuning_dir.exists():
        ...
        print()
        print("請確認：")
        print("  1. 是否已執行過 tune_setC_v2.py")
    ...
    try:
        ...
        for i, trial in enumerate(best_trials[:5], 1):
            ...
            for key, value in trial.hyperparameters.values.items():
                if key == "units":
                    num_layers = trial.hyperparameters.values.get("num_layers", 2)
                    print(f"    - {key}: {value} (綁定，所有 {num_layers} 層相同)")
                else:
                    print(f"    - {key}: {value}")
        ...
        model_name = f"best_tuned_setC_v2_partial_{timestamp}"
        ...
        save_model_config(
            ...
            additional_params={
                ...
                "model_type": "tuned_setC_v2_partial",
                "optimization_notes": [
                    "綁定所有 LSTM 層使用相同單元數",
                    "Dropout 下限提高至 0.25",
                    "學習率範圍擴展至 0.00001",
        ...
        print(f"  - 綁定單元數: {units} (所有層相同)")
        print(f"  - Dropout: {best_hps.get('dropout_rate')}")
        print(f"  - 學習率: {best_hps.get('learning_rate')}")
        print()
        print("優化策略:")
        print("  1. 綁定所有 LSTM 層使用相同單元數")
        print("  2. Dropout 下限提高至 0.25")
        print("  3. 學習率範圍擴展至 0.00001")
        print("  4. Early Stopping patience 提高至 15")
        print()
        print("輸出檔案:")
        ...
        print("   如果想要更好的結果，可以繼續執行 tune_setC_v2.py")
        print("=" * 80)
